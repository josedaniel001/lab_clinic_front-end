...
export default function OrdenesPage() {
  ...
  const fetchPacientes = async () => {
    const { results } = await pacientesAPI.getPacientes()
    console.log("carga de pacientes: "+ JSON.stringify(results,null,2));
    setPacientes(Array.isArray(results) ? results : [])
  }
  ...
  const validateStep = () => {
    const newErrors = { id_paciente: "", id_medico: "", hora_entrega: "", horas_ayuno: "", examenes: "" }
    let isValid = true

    if (currentStep === 0) {
      if (!formData.id_paciente) { newErrors.id_paciente = "Seleccione un paciente"; isValid = false }
  ...
  const handleSubmit = async () => {
    ...
    try {
        const payload = {
          paciente: parseInt(formData.id_paciente),
  ...
  const filteredOrdenes = ordenes.filter(o =>
    o.paciente_nombre.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const columns = [
    ...
    { key: "paciente_nombre", label: "Paciente", className: "font-medium text-gray-900" },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Combobox
                    ...
                    onChange={(val) =>{
                        console.log("Paciente seleccionado:", val) // ✅ Debe imprimirse
                        handleChange("id_paciente", val)
                      }}
            ...
            <div className="space-y-4">
              <Card>
                ...
                <CardContent>
                      ...
                      {(() => {
                        const p = pacientes.find(p => String(p.id) === formData.id_paciente)
                        return p ? `${p.nombres} ${p.apellidos}` : "No seleccionado"
                      })()}
                    </p>
                    <p>
      ...
      <DialogContent className="max-w-2xl">
        ...
        {selectedOrden && (
          <div className="space-y-4">
            ...
            <p><strong>Paciente:</strong> {selectedOrden.paciente_nombre}</p>
