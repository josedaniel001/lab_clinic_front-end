  ...
  const columns = [
    {
      ...
      render: (value: any, row: any) => (
        <Avatar className="h-10 w-10">
          <AvatarImage src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${row.nombre}`} />
          <AvatarFallback className="font-semibold text-sm" style={{ backgroundColor: "#dbeafe", color: "#1e40af" }}>
            {row.nombre
              .split(" ")
              .map((n: string) => n[0])
              .join("")
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <Avatar className="h-16 w-16">
                  <AvatarImage src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${selectedMedico.nombre}`} />
                  <AvatarFallback
